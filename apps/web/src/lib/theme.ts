...
export const THEME_INIT_SCRIPT = `(function(){try{if(window.location.pathname==='/')return;var K='afe-theme-colors';var M='afe-theme-mode';var L={'accent':'#F0FFF7','accent-foreground':'#00AA66','background':'#FFFFFF','border':'#E5E7EB','card':'#F9FAFB','card-foreground':'#111827','destructive':'#DC2626','destructive-foreground':'#FFFFFF','foreground':'#111827','input':'#F3F4F6','muted':'#F3F4F6','muted-foreground':'#6B7280','popover':'#FFFFFF','popover-foreground':'#111827','primary':'#00CC7A','primary-foreground':'#FFFFFF','ring':'#00CC7A','secondary':'#F3F4F6','secondary-foreground':'#111827','sidebar-accent':'#00CC7A','sidebar-accent-foreground':'#FFFFFF','sidebar-background':'#F9FAFB','sidebar-border':'#E5E7EB','sidebar-foreground':'#111827','sidebar-primary':'#00CC7A','sidebar-primary-foreground':'#FFFFFF','sidebar-ring':'#00CC7A'};var D={'accent':'#1E1E2E','accent-foreground':'#D9F99D','background':'#0D0D0D','border':'#2A2A2A','card':'#111111','card-foreground':'#EDEDED','destructive':'#FF4C4C','destructive-foreground':'#FFFFFF','foreground':'#EDEDED','input':'#1F1F1F','muted':'#202020','muted-foreground':'#8B8B8B','popover':'#111111','popover-foreground':'#EDEDED','primary':'#00FF9C','primary-foreground':'#0D0D0D','ring':'#00FF9C','secondary':'#1E1E1E','secondary-foreground':'#EDEDED','sidebar-accent':'#00FF9C','sidebar-accent-foreground':'#0D0D0D','sidebar-background':'#0B0B0B','sidebar-border':'#2A2A2A','sidebar-foreground':'#EDEDED','sidebar-primary':'#00FF9C','sidebar-primary-foreground':'#0D0D0D','sidebar-ring':'#38BDF8'};function h(x){var r=parseInt(x.slice(1,3),16)/255,g=parseInt(x.slice(3,5),16)/255,b=parseInt(x.slice(5,7),16)/255,mx=Math.max(r,g,b),mn=Math.min(r,g,b),hh=0,s=0,l=(mx+mn)/2;if(mx!==mn){var d=mx-mn;s=l>.5?d/(2-mx-mn):d/(mx+mn);if(mx===r)hh=((g-b)/d+(g<b?6:0))/6;else if(mx===g)hh=((b-r)/d+2)/6;else hh=((r-g)/d+4)/6;}return Math.round(hh*360)+' '+Math.round(s*100)+'% '+Math.round(l*100)+'%';}function a(c){var el=document.documentElement;for(var k in c){var v=c[k];if(v&&v[0]==='#')el.style.setProperty('--'+k,h(v));else if(v)el.style.setProperty('--'+k,v);}}var mode=localStorage.getItem(M);var stored=JSON.parse(localStorage.getItem(K)||'null');if(stored&&mode!=='system'){a(stored);return;}var dark=window.matchMedia&&window.matchMedia('(prefers-color-scheme:dark)').matches;a(dark?D:L);}catch(e){}})();`;
