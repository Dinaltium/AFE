  ...
  return (
    ...
    >
      ...
      <header
        ...
        style={{ background: "rgba(10,10,10,0.85)", borderColor: "#1A1A1A", backdropFilter: "blur(12px)" }}
      >
        <Link href="/" className="text-xl font-bold tracking-tight" style={{ color: "#00FF9C" }}>
          AFE
        </Link>
        <nav className="hidden md:flex items-center gap-6 text-sm" style={{ color: "#8B8B8B" }}>
        ...
        <div className="flex items-center gap-3">
          <Link href="/login">
            <Button variant="ghost" size="sm" style={{ color: "#8B8B8B" }} className="hover:text-white">
              Log in
            </Button>
          </Link>
          <Link href="/signup">
      ...
      <section className="flex flex-col items-center justify-center min-h-screen text-center px-6 pt-24 pb-16 relative overflow-hidden">
        ...
        <div
          ...
          style={{ background: "radial-gradient(circle, rgba(0,255,156,0.07) 0%, transparent 70%)" }}
        />
        ...
        >
          <div
            ...
            style={{ borderColor: "#00FF9C33", color: "#00FF9C", background: "#00FF9C0D" }}
          >
          ...
          >
            Your finances,{" "}
            <span style={{ color: "#00FF9C" }}>on autopilot.</span>
          </h1>

          <p className="text-lg md:text-xl max-w-2xl mx-auto mb-10" style={{ color: "#8B8B8B" }}>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <a href="#how-it-works">
              <Button
                ...
                style={{ borderColor: "#2A2A2A", color: "#EDEDED", background: "transparent" }}
              >
                See how it works
      ...
      {/* Stats */}
      <section className="border-y py-12" style={{ borderColor: "#1A1A1A", background: "#0D0D0D" }}>
        <div className="max-w-5xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {STATS.map(({ icon: Icon, value, label }) => (
            <div key={label}>
              <Icon className="mx-auto mb-2 h-5 w-5" style={{ color: "#00FF9C" }} />
              <div className="text-3xl font-bold mb-1" style={{ color: "#EDEDED" }}>{value}</div>
              <div className="text-sm" style={{ color: "#8B8B8B" }}>{label}</div>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto">
          <Section>
            <motion.div variants={fadeUp} className="text-center mb-16">
              <h2 className="text-3xl md:text-4xl font-bold mb-4" style={{ color: "#EDEDED" }}>
              ...
              {FEATURES.map(({ icon: Icon, title, description }) => (
                ...
                >
                  <div
                    ...
                    style={{ background: "#00FF9C1A" }}
                  >
                    <Icon className="h-5 w-5" style={{ color: "#00FF9C" }} />
                  </div>
                  <h3 className="font-semibold text-lg mb-2" style={{ color: "#EDEDED" }}>{title}</h3>
            ...
            <div className="relative">
              ...
              <div className="space-y-12">
                {STEPS.map(({ number, title, description }) => (
                  ...
                  >
                    <div
                      className="w-14 h-14 rounded-full flex-shrink-0 flex items-center justify-center font-mono text-sm font-bold border-2"
                      style={{ borderColor: "#00FF9C", color: "#00FF9C", background: "#0A0A0A" }}
                    >
                      {number}
                    </div>
                    <div className="pt-3">
                      <h3 className="font-semibold text-xl mb-2" style={{ color: "#EDEDED" }}>{title}</h3>
                      <p className="text-base" style={{ color: "#8B8B8B" }}>{description}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
              ...
              {TESTIMONIALS.map(({ icon: Icon, role, name, text }) => (
                ...
                >
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <div className="font-medium text-sm" style={{ color: "#EDEDED" }}>{name}</div>
                      <div className="text-xs" style={{ color: "#8B8B8B" }}>{role}</div>
                    </div>
                  </div>
                  <p className="text-sm leading-relaxed" style={{ color: "#8B8B8B" }}>{text}</p>
                </motion.div>
              ))}
            </div>
          </Section>
      ...
      {/* CTA */}
      <section className="py-24 px-6 text-center" style={{ background: "#0D0D0D", borderTop: "1px solid #1A1A1A" }}>
        <Section>
          <motion.div variants={fadeUp} className="max-w-2xl mx-auto">
            <h2 className="text-4xl md:text-5xl font-bold mb-6" style={{ color: "#EDEDED" }}>
              Start for free today.
            </h2>
            <p className="text-lg mb-10" style={{ color: "#8B8B8B" }}>
      ...
      {/* Footer */}
      <footer className="border-t py-12 px-6" style={{ borderColor: "#1A1A1A", background: "#0A0A0A" }}>
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <div className="text-xl font-bold mb-1" style={{ color: "#00FF9C" }}>AFE</div>
            <div className="text-sm" style={{ color: "#8B8B8B" }}>
              Autonomous Finance Engine for the gig economy.
            </div>
          </div>
          <nav className="flex gap-6 text-sm" style={{ color: "#8B8B8B" }}>
